...
### LLM 호출 배지 (v0.8.0+ Debate/Audit 분리)

`config.json logging.show_transport_badge: true` (기본) 일 때, Phase 1 R1 시작 직전에 전체 Performer 의 Transport/Model 현황을 출력한다. **opus 는 토론에서 보이지 않는다** (금지선):
...
Phase 3 Audit 시작 직전에도 감사 순서 배지를 출력한다 — **final-auditor 의 opus 위치가 여기서만 드러난다**:
...
배지에 API 키·인증 토큰은 절대 포함하지 않는다. 상세 규약은 `CONTRACT.md §8.6` + `§11.3` 참조.
...
## 출력 포맷
최종 사용자 응답은 다음 구조 (v0.8.0+ 는 `unanimous` 필드 포함):
```
## Ensembra Run — {preset}
