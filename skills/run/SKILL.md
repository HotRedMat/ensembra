...
### Transport 호출 규약 (architect = Ollama, v0.6.0+)

Phase 1 R1 에서 architect Performer 를 호출할 때 기본 Transport 는 Ollama 다. `userConfig.ollama_endpoint` (비시크릿) 치환으로 엔드포인트를 얻는다:
...
**Ollama 가용 실패** (endpoint 응답 없음·모델 미존재):
- architect Performer 는 즉시 Claude 서브에이전트로 폴백
- Conductor 는 출력 상단에 배지 표시: `⚠ architect: ollama → claude-sonnet (fallback)`
- 파이프라인은 정상 진행

**Gemini 경로 제거**: v0.5.x 에서 기본이던 Gemini Transport 는 v0.6.0 에서 폐지됨. `userConfig.gemini_api_key` 는 `sensitive: true` 로 복구되어 skill/agent content 에서 접근 불가. 재도입은 MCP server / hook 기반 architect 재설계 후 (Gate3 이월). 상세 이유는 `CONTRACT.md §8.4` 및 `CHANGELOG.md [0.6.0]` 참조.
