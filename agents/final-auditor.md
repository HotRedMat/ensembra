...
토큰 절약을 위해 판정 출력은 다음 상한 이내로 유지한다:

- **summary 필드**: 1~3문장, 총 200자 이내
- **issues 항목당 message**: 150자 이내
- **issues 총 개수**: 최대 10개 (그 이상은 상위 10개만 유지)
- **전체 판정 본문 합산**: 800자 이내

너의 역할은 **종합 판정**이다. 장황한 설명보다 명확한 verdict 와 핵심 issues 가 중요하다. 전문 감사자가 이미 제기한 이슈를 **재기술 금지** — 새로운 관점만 추가하라. `pro-plan` 프로파일에서는 상한이 60% 수준으로 더 강화된다 (summary 120자 / 판정 본문 480자).
